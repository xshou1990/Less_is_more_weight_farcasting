...
torch.manual_seed(0)
maxval = 21
t = np.random.randint(low=maxval//2, high=maxval)
#t = 20
print(t)
...
for m in range(5):
    ...
    data = np.load("dataset/Neural_Network_1d_Adam2/Sample_{}/sample.npy".format(m+1))

    data_tr = data[:100,:,:]
    data_val = data[100:150,:,:]
    data_te = data[150:,:,:]

    #print(data_tr.shape[2])

    def get_batch_n(data,batch_size):
    #     if data.shape[2] > 20:
    ...
    tr_W1, tr_W2 = get_batch_n(data_tr,len(data_tr))
    tr_W1, tr_W2  = np.transpose(tr_W1, (0,2,1)),np.transpose(tr_W2, (0,2,1))
    val_W1, val_W2 = get_batch_n(data_val,len(data_val))
    val_W1, val_W2  = np.transpose(val_W1, (0,2,1)),np.transpose(val_W2, (0,2,1))
    te_W1, te_W2 = get_batch_n(data_te,len(data_te))
