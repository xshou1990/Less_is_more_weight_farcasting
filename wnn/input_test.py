...
#         X1_tf = np.reshape(X1_tf, (1,5))
        X1_pt = torch.tensor([1.,2.,3.,4.,5.,6.])
        X1_pt = torch.reshape(X1_pt, (6,))
...
#     print(X2_tf.shape)
    ...
    X2_pt = X1_pt[0:5] - X1_pt[1:6]
    X2_pt = torch.reshape(X2_pt, (5,))
    print("PT shape:")
    print(X1_pt.shape)
    print(X2_pt.shape)
...
input_test(X1_tf = 0, X1_pt = 0, fc = False, conv = False, bias = False, random = False)